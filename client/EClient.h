...
// deltaNeutralClearingIntent in openOrder 53 = can receive orderRef in execution 54 = can receive scale order fields
// (PriceAdjustValue, PriceAdjustInterval, ProfitOffset, AutoReset,
...
public:
    ...
    void reqHistoricalData(TickerId id, const Contract& contract, const std::string& endDateTime,
    ...
    void reqSecDefOptParams(int reqId, const std::string& underlyingSymbol, const std::string& futFopExchange,
    ...
    void reqHistoricalNews(int requestId, int conId, const std::string& providerCodes, const std::string& startDateTime,
        const std::string& endDateTime, int totalResults, const TagValueListSPtr& historicalNewsOptions);
...
template <>
void EClient::EncodeField<Decimal>(std::ostream& os, Decimal);
template <>
void EClient::EncodeField<std::string>(std::ostream& os, std::string);
