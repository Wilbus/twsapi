...
const char* EDecoder::processTickOptionComputationMsg(const char* ptr, const char* endPtr)
{
    ...
    if (version >= 6 || tickTypeInt == MODEL_OPTION || tickTypeInt == DELAYED_MODEL_OPTION_COMPUTATION)
    ...
    m_pEWrapper->tickOptionComputation(tickerId, (TickType)tickTypeInt, tickAttrib, impliedVol, delta, optPrice,
...
const char* EDecoder::processTickEfpMsg(const char* ptr, const char* endPtr)
{
    ...
    m_pEWrapper->tickEFP(tickerId, (TickType)tickTypeInt, basisPoints, formattedBasisPoints, impliedFuturesPrice,
...
const char* EDecoder::processOrderStatusMsg(const char* ptr, const char* endPtr)
{
    ...
    m_pEWrapper->orderStatus(orderId, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice,
...
const char* EDecoder::processOpenOrderMsg(const char* ptr, const char* endPtr)
{
    ...
    // read order fields
    bool success =
        eOrderDecoder.decodeAction(ptr, endPtr) && eOrderDecoder.decodeTotalQuantity(ptr, endPtr) &&
        eOrderDecoder.decodeOrderType(ptr, endPtr) && eOrderDecoder.decodeLmtPrice(ptr, endPtr) &&
        eOrderDecoder.decodeAuxPrice(ptr, endPtr) && eOrderDecoder.decodeTIF(ptr, endPtr) &&
        eOrderDecoder.decodeOcaGroup(ptr, endPtr) && eOrderDecoder.decodeAccount(ptr, endPtr) &&
        eOrderDecoder.decodeOpenClose(ptr, endPtr) && eOrderDecoder.decodeOrigin(ptr, endPtr) &&
        eOrderDecoder.decodeOrderRef(ptr, endPtr) && eOrderDecoder.decodeClientId(ptr, endPtr) &&
        eOrderDecoder.decodePermId(ptr, endPtr) && eOrderDecoder.decodeOutsideRth(ptr, endPtr) &&
        eOrderDecoder.decodeHidden(ptr, endPtr) && eOrderDecoder.decodeDiscretionaryAmount(ptr, endPtr) &&
        eOrderDecoder.decodeGoodAfterTime(ptr, endPtr) && eOrderDecoder.skipSharesAllocation(ptr, endPtr) &&
        eOrderDecoder.decodeFAParams(ptr, endPtr) && eOrderDecoder.decodeModelCode(ptr, endPtr) &&
        eOrderDecoder.decodeGoodTillDate(ptr, endPtr) && eOrderDecoder.decodeRule80A(ptr, endPtr) &&
        eOrderDecoder.decodePercentOffset(ptr, endPtr) && eOrderDecoder.decodeSettlingFirm(ptr, endPtr) &&
        eOrderDecoder.decodeShortSaleParams(ptr, endPtr) && eOrderDecoder.decodeAuctionStrategy(ptr, endPtr) &&
        eOrderDecoder.decodeBoxOrderParams(ptr, endPtr) && eOrderDecoder.decodePegToStkOrVolOrderParams(ptr, endPtr) &&
        eOrderDecoder.decodeDisplaySize(ptr, endPtr) && eOrderDecoder.decodeBlockOrder(ptr, endPtr) &&
        eOrderDecoder.decodeSweepToFill(ptr, endPtr) && eOrderDecoder.decodeAllOrNone(ptr, endPtr) &&
        eOrderDecoder.decodeMinQty(ptr, endPtr) && eOrderDecoder.decodeOcaType(ptr, endPtr) &&
        eOrderDecoder.skipETradeOnly(ptr, endPtr) && eOrderDecoder.skipFirmQuoteOnly(ptr, endPtr) &&
        eOrderDecoder.skipNbboPriceCap(ptr, endPtr) && eOrderDecoder.decodeParentId(ptr, endPtr) &&
        eOrderDecoder.decodeTriggerMethod(ptr, endPtr) && eOrderDecoder.decodeVolOrderParams(ptr, endPtr, true) &&
        eOrderDecoder.decodeTrailParams(ptr, endPtr) && eOrderDecoder.decodeBasisPoints(ptr, endPtr) &&
        eOrderDecoder.decodeComboLegs(ptr, endPtr) && eOrderDecoder.decodeSmartComboRoutingParams(ptr, endPtr) &&
        eOrderDecoder.decodeScaleOrderParams(ptr, endPtr) && eOrderDecoder.decodeHedgeParams(ptr, endPtr) &&
        eOrderDecoder.decodeOptOutSmartRouting(ptr, endPtr) && eOrderDecoder.decodeClearingParams(ptr, endPtr) &&
        eOrderDecoder.decodeNotHeld(ptr, endPtr) && eOrderDecoder.decodeDeltaNeutral(ptr, endPtr) &&
        eOrderDecoder.decodeAlgoParams(ptr, endPtr) && eOrderDecoder.decodeSolicited(ptr, endPtr) &&
        eOrderDecoder.decodeWhatIfInfoAndCommission(ptr, endPtr) &&
        eOrderDecoder.decodeVolRandomizeFlags(ptr, endPtr) && eOrderDecoder.decodePegBenchParams(ptr, endPtr) &&
        eOrderDecoder.decodeConditions(ptr, endPtr) && eOrderDecoder.decodeAdjustedOrderParams(ptr, endPtr) &&
        eOrderDecoder.decodeSoftDollarTier(ptr, endPtr) && eOrderDecoder.decodeCashQty(ptr, endPtr) &&
        eOrderDecoder.decodeDontUseAutoPriceForHedge(ptr, endPtr) && eOrderDecoder.decodeIsOmsContainer(ptr, endPtr) &&
        eOrderDecoder.decodeDiscretionaryUpToLimitPrice(ptr, endPtr) &&
        eOrderDecoder.decodeUsePriceMgmtAlgo(ptr, endPtr) && eOrderDecoder.decodeDuration(ptr, endPtr) &&
...
const char* EDecoder::processContractDataMsg(const char* ptr, const char* endPtr)
{
    ...
    if (m_serverVersion >= MIN_SERVER_VER_FUND_DATA_FIELDS && contract.contract.secType == "FUND")
    {
        ...
        contract.fundDistributionPolicyIndicator =
            Utils::getFundDistributionPolicyIndicator(fundDistributionPolicyIndicator);
...
const char* EDecoder::processMarketDepthL2Msg(const char* ptr, const char* endPtr)
{
    ...
    m_pEWrapper->updateMktDepthL2(id, position, marketMaker, operation, side, price, size, isSmartDepth);
...
const char* EDecoder::processScannerDataMsg(const char* ptr, const char* endPtr)
{
    ...
    for (int ctr = 0; ctr < numberOfElements; ++ctr)
    {
        ...
        m_pEWrapper->scannerData(
            tickerId, data.rank, data.contract, data.distance, data.benchmark, data.projection, data.legsStr);
    }
...
const char* EDecoder::processSecurityDefinitionOptionalParameterMsg(const char* ptr, const char* endPtr)
{
    ...
    m_pEWrapper->securityDefinitionOptionalParameter(
...
const char* EDecoder::processCompletedOrderMsg(const char* ptr, const char* endPtr)
{
    ...
    bool success =
        ...
        eOrderDecoder.decodeOcaGroup(ptr, endPtr) && eOrderDecoder.decodeAccount(ptr, endPtr) &&
        eOrderDecoder.decodeOpenClose(ptr, endPtr) && eOrderDecoder.decodeOrigin(ptr, endPtr) &&
        eOrderDecoder.decodeOrderRef(ptr, endPtr) && eOrderDecoder.decodePermId(ptr, endPtr) &&
        eOrderDecoder.decodeOutsideRth(ptr, endPtr) && eOrderDecoder.decodeHidden(ptr, endPtr) &&
        eOrderDecoder.decodeDiscretionaryAmount(ptr, endPtr) && eOrderDecoder.decodeGoodAfterTime(ptr, endPtr) &&
        eOrderDecoder.decodeFAParams(ptr, endPtr) && eOrderDecoder.decodeModelCode(ptr, endPtr) &&
        eOrderDecoder.decodeGoodTillDate(ptr, endPtr) && eOrderDecoder.decodeRule80A(ptr, endPtr) &&
        eOrderDecoder.decodePercentOffset(ptr, endPtr) && eOrderDecoder.decodeSettlingFirm(ptr, endPtr) &&
        eOrderDecoder.decodeShortSaleParams(ptr, endPtr) && eOrderDecoder.decodeBoxOrderParams(ptr, endPtr) &&
        eOrderDecoder.decodePegToStkOrVolOrderParams(ptr, endPtr) && eOrderDecoder.decodeDisplaySize(ptr, endPtr) &&
        eOrderDecoder.decodeSweepToFill(ptr, endPtr) && eOrderDecoder.decodeAllOrNone(ptr, endPtr) &&
        eOrderDecoder.decodeMinQty(ptr, endPtr) && eOrderDecoder.decodeOcaType(ptr, endPtr) &&
        eOrderDecoder.decodeTriggerMethod(ptr, endPtr) && eOrderDecoder.decodeVolOrderParams(ptr, endPtr, false) &&
        eOrderDecoder.decodeTrailParams(ptr, endPtr) && eOrderDecoder.decodeComboLegs(ptr, endPtr) &&
        eOrderDecoder.decodeSmartComboRoutingParams(ptr, endPtr) && eOrderDecoder.decodeScaleOrderParams(ptr, endPtr) &&
        eOrderDecoder.decodeHedgeParams(ptr, endPtr) && eOrderDecoder.decodeClearingParams(ptr, endPtr) &&
        eOrderDecoder.decodeNotHeld(ptr, endPtr) && eOrderDecoder.decodeDeltaNeutral(ptr, endPtr) &&
        eOrderDecoder.decodeAlgoParams(ptr, endPtr) && eOrderDecoder.decodeSolicited(ptr, endPtr) &&
        eOrderDecoder.decodeOrderStatus(ptr, endPtr) && eOrderDecoder.decodeVolRandomizeFlags(ptr, endPtr) &&
        eOrderDecoder.decodePegBenchParams(ptr, endPtr) && eOrderDecoder.decodeConditions(ptr, endPtr) &&
        eOrderDecoder.decodeStopPriceAndLmtPriceOffset(ptr, endPtr) && eOrderDecoder.decodeCashQty(ptr, endPtr) &&
        eOrderDecoder.decodeDontUseAutoPriceForHedge(ptr, endPtr) && eOrderDecoder.decodeIsOmsContainer(ptr, endPtr) &&
        eOrderDecoder.decodeAutoCancelDate(ptr, endPtr) && eOrderDecoder.decodeFilledQuantity(ptr, endPtr) &&
        eOrderDecoder.decodeRefFuturesConId(ptr, endPtr) && eOrderDecoder.decodeAutoCancelParent(ptr, endPtr) &&
        eOrderDecoder.decodeShareholder(ptr, endPtr) && eOrderDecoder.decodeImbalanceOnly(ptr, endPtr) &&
        eOrderDecoder.decodeRouteMarketableToBbo(ptr, endPtr) && eOrderDecoder.decodeParentPermId(ptr, endPtr) &&
        eOrderDecoder.decodeCompletedTime(ptr, endPtr) && eOrderDecoder.decodeCompletedStatus(ptr, endPtr) &&
        eOrderDecoder.decodePegBestPegMidOrderAttributes(ptr, endPtr);
...
bool EDecoder::DecodeField(std::string& stringValue, const char*& ptr, const char* endPtr)
{
    if (!CheckOffset(ptr, endPtr))
        return false;
    const char* fieldBeg = ptr;
...
bool EDecoder::DecodeField(char& charValue, const char*& ptr, const char* endPtr)
{
    if (!CheckOffset(ptr, endPtr))
        return false;
    const char* fieldBeg = ptr;
