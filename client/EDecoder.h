...
const int MAX_CLIENT_VER = MIN_SERVER_VER_MANUAL_ORDER_TIME_EXERCISE_OPTIONS;
...
#define DECODE_FIELD(x)                         \
    if (!EDecoder::DecodeField(x, ptr, endPtr)) \
        return 0;
#define DECODE_FIELD_TIME(x)                        \
    if (!EDecoder::DecodeFieldTime(x, ptr, endPtr)) \
        return 0;
#define DECODE_FIELD_MAX(x)                        \
    if (!EDecoder::DecodeFieldMax(x, ptr, endPtr)) \
        return 0;
#endif
