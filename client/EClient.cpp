...
bool EClient::isAsciiPrintable(const std::string& s)
{
    return std::all_of(s.begin(), s.end(), [](char c) {
        return (static_cast<unsigned char>(c) >= 32 && static_cast<unsigned char>(c) < 127) ||
               static_cast<unsigned char>(c) == 9 || static_cast<unsigned char>(c) == 10 ||
               static_cast<unsigned char>(c) == 13;
    });
}
...
void EClient::reqMktData(TickerId tickerId, const Contract& contract, const std::string& genericTicks, bool snapshot,
    bool regulatorySnaphsot, const TagValueListSPtr& mktDataOptions)
{
    // not connected?
    if (!isConnected())
    {
    ...
    if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL)
    {
        ...
        {
            m_pEWrapper->error(
                tickerId, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support delta-neutral orders.", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_REQ_MKT_DATA_CONID)
    {
        ...
        {
            m_pEWrapper->error(
                tickerId, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support conId parameter.", "");
            return;
        }
    }
...
void EClient::reqMktDepth(TickerId tickerId, const Contract& contract, int numRows, bool isSmartDepth,
    const TagValueListSPtr& mktDepthOptions)
{
    ...
    {
        m_pEWrapper->error(
            tickerId, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support SMART depth request.", "");
        return;
    }
...
void EClient::cancelMktDepth(TickerId tickerId, bool isSmartDepth)
{
    ...
    {
        m_pEWrapper->error(
            tickerId, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support SMART depth cancel.", "");
        return;
    }
...
void EClient::reqHistoricalData(TickerId tickerId, const Contract& contract, const std::string& endDateTime,
    const std::string& durationStr, const std::string& barSizeSetting, const std::string& whatToShow, int useRTH,
    int formatDate, bool keepUpToDate, const TagValueListSPtr& chartOptions)
{
    // not connected?
    if (!isConnected())
    {
...
void EClient::reqRealTimeBars(TickerId tickerId, const Contract& contract, int barSize, const std::string& whatToShow,
    bool useRTH, const TagValueListSPtr& realTimeBarsOptions)
{
    // not connected?
    if (!isConnected())
    {
...
void EClient::reqScannerSubscription(int tickerId, const ScannerSubscription& subscription,
...
void EClient::reqFundamentalData(TickerId reqId, const Contract& contract, const std::string& reportType,
    ...
    const TagValueListSPtr& fundamentalDataOptions)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support fundamental data requests.", "");
        return;
    }
...
void EClient::calculateImpliedVolatility(TickerId reqId, const Contract& contract, double optionPrice,
    double underPrice,
    // reserved for future use, must be blank
    ...
    const TagValueListSPtr& miscOptions)
{
    ...
    {
        m_pEWrapper->error(
            reqId, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support calculate option price requests.", "");
        return;
    }
...
void EClient::placeOrder(OrderId id, const Contract& contract, const Order& order)
{
    ...
    if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support delta-neutral orders.", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_ALGO_ORDERS)
    {
        ...
        {
            m_pEWrapper->error(id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support algo orders.", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_NOT_HELD)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support notHeld parameter.", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_SEC_ID_TYPE)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support secIdType and secId parameters.", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_PLACE_ORDER_CONID)
    {
        if (contract.conId > 0)
        {
            m_pEWrapper->error(id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support conId parameter.", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_SSHORTX)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support exemptCode parameter.", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_HEDGE_ORDERS)
    {
        ...
        {
            m_pEWrapper->error(id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support hedge orders.", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_OPT_OUT_SMART_ROUTING)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support optOutSmartRouting parameter.", "");
            return;
        }
    }
    ...
    {
        if (order.deltaNeutralConId > 0 || !order.deltaNeutralSettlingFirm.empty() ||
            !order.deltaNeutralClearingAccount.empty() || !order.deltaNeutralClearingIntent.empty())
        {
            m_pEWrapper->error(id, UPDATE_TWS.code(),
                UPDATE_TWS.msg() + "  It does not support deltaNeutral parameters: ConId, SettlingFirm, "
    ...
    {
        if (!order.deltaNeutralOpenClose.empty() || order.deltaNeutralShortSale ||
            order.deltaNeutralShortSaleSlot > 0 || !order.deltaNeutralDesignatedLocation.empty())
        {
            m_pEWrapper->error(id, UPDATE_TWS.code(),
                UPDATE_TWS.msg() + "  It does not support deltaNeutral parameters: OpenClose, ShortSale, "
    ...
    if (m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS3)
    {
        ...
        {
            if (order.scalePriceAdjustValue != UNSET_DOUBLE || order.scalePriceAdjustInterval != UNSET_INTEGER ||
                order.scaleProfitOffset != UNSET_DOUBLE || order.scaleAutoReset ||
                order.scaleInitPosition != UNSET_INTEGER || order.scaleInitFillQty != UNSET_INTEGER ||
    ...
    if (m_serverVersion < MIN_SERVER_VER_TRAILING_PERCENT)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support trailing percent parameter", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_ALGO_ID)
    {
        ...
        {
            m_pEWrapper->error(id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support algoId parameter", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_ORDER_SOLICITED)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support order solicited parameter.", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_MODELS_SUPPORT)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support model code parameter.", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_EXT_OPERATOR)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support ext operator parameter", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_SOFT_DOLLAR_TIER)
    {
        ...
        {
            m_pEWrapper->error(id, UPDATE_TWS.code(), UPDATE_TWS.msg() + " It does not support soft dollar tier", "");
            return;
        }
    }
    ...
    if (m_serverVersion < MIN_SERVER_VER_CASH_QTY)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support cash quantity parameter", "");
            return;
        }
    }

    if (m_serverVersion < MIN_SERVER_VER_DECISION_MAKER &&
        (!order.mifid2DecisionMaker.empty() || !order.mifid2DecisionAlgo.empty()))
    {
        m_pEWrapper->error(
            id, UPDATE_TWS.code(), UPDATE_TWS.msg() + " It does not support MIFID II decision maker parameters", "");
        return;
    }

    if (m_serverVersion < MIN_SERVER_VER_MIFID_EXECUTION &&
        (!order.mifid2ExecutionTrader.empty() || !order.mifid2ExecutionAlgo.empty()))
    {
        m_pEWrapper->error(
            id, UPDATE_TWS.code(), UPDATE_TWS.msg() + " It does not support MIFID II execution parameters", "");
        return;
    }

    if (m_serverVersion < MIN_SERVER_VER_AUTO_PRICE_FOR_HEDGE && order.dontUseAutoPriceForHedge)
    {
        m_pEWrapper->error(id, UPDATE_TWS.code(),
    ...
    if (m_serverVersion < MIN_SERVER_VER_ORDER_CONTAINER && order.isOmsContainer)
    {
        m_pEWrapper->error(
            id, UPDATE_TWS.code(), UPDATE_TWS.msg() + " It does not support oms container parameter", "");
        return;
    }

    if (m_serverVersion < MIN_SERVER_VER_D_PEG_ORDERS && order.discretionaryUpToLimitPrice)
    {
        m_pEWrapper->error(id, UPDATE_TWS.code(), UPDATE_TWS.msg() + " It does not support D-Peg orders", "");
        return;
    }

    if (m_serverVersion < MIN_SERVER_VER_PRICE_MGMT_ALGO && order.usePriceMgmtAlgo != UsePriceMmgtAlgo::DEFAULT)
    {
        m_pEWrapper->error(
            id, UPDATE_TWS.code(), UPDATE_TWS.msg() + " It does not support Use Price Management Algo requests", "");

        return;
    }

    if (m_serverVersion < MIN_SERVER_VER_DURATION && order.duration != UNSET_INTEGER)
    {
    ...
    if (m_serverVersion < MIN_SERVER_VER_POST_TO_ATS && order.postToAts != UNSET_INTEGER)
    {
    ...
    if (m_serverVersion < MIN_SERVER_VER_AUTO_CANCEL_PARENT)
    {
        ...
        {
            m_pEWrapper->error(
                id, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support autoCancelParent parameter.", "");
            return;
        }
    }
    ...
    {
        m_pEWrapper->error(
            id, UPDATE_TWS.code(), UPDATE_TWS.msg() + " It does not support advanced error override attribute", "");
        return;
    }
    ...
    {
        m_pEWrapper->error(
            id, UPDATE_TWS.code(), UPDATE_TWS.msg() + " It does not support manual order time attribute", "");
        return;
    }

    if (m_serverVersion < MIN_SERVER_VER_PEGBEST_PEGMID_OFFSETS)
    {
        if (order.minTradeQty != UNSET_INTEGER || order.minCompeteSize != UNSET_INTEGER ||
            order.competeAgainstBestOffset != UNSET_DOUBLE || order.midOffsetAtWhole != UNSET_DOUBLE ||
            order.midOffsetAtHalf != UNSET_DOUBLE)
        {
            m_pEWrapper->error(id, UPDATE_TWS.code(),
                UPDATE_TWS.msg() + "  It does not support PEG BEST / PEG MID order parameters: minTradeQty, "
                                   "minCompeteSize, competeAgainstBestOffset, midOffsetAtWhole and midOffsetAtHalf",
                "");
            return;
        }
    }
    ...
    try
    {
        ...
        if (m_serverVersion >= MIN_SERVER_VER_SCALE_ORDERS3 && order.scalePriceIncrement > 0.0 &&
            order.scalePriceIncrement != UNSET_DOUBLE)
        {
...
void EClient::cancelOrder(OrderId id, const std::string& manualOrderCancelTime)
{
    ...
    {
        m_pEWrapper->error(
            id, UPDATE_TWS.code(), UPDATE_TWS.msg() + " It does not support manual order cancel time attribute", "");
        return;
    }
...
void EClient::exerciseOptions(TickerId tickerId, const Contract& contract, int exerciseAction, int exerciseQuantity,
...
void EClient::reqGlobalCancel()
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support globalCancel requests.", "");
        return;
    }
...
void EClient::reqMarketDataType(int marketDataType)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support market data type requests.", "");
        return;
    }
...
void EClient::reqPositions()
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support positions request.", "");
        return;
    }
...
void EClient::cancelPositions()
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support positions cancellation.", "");
        return;
    }
...
void EClient::reqAccountSummary(int reqId, const std::string& groupName, const std::string& tags)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support account summary request.", "");
        return;
    }
...
void EClient::verifyRequest(const std::string& apiName, const std::string& apiVersion)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support verification request.", "");
        return;
    }
...
void EClient::verifyAndAuthRequest(
    const std::string& apiName, const std::string& apiVersion, const std::string& opaqueIsvKey)
{
    // not connected?
    if (!isConnected())
    {
...
void EClient::queryDisplayGroups(int reqId)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support queryDisplayGroups request.", "");
        return;
    }
...
void EClient::updateDisplayGroup(int reqId, const std::string& contractInfo)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support updateDisplayGroup request.", "");
        return;
    }
...
void EClient::reqPositionsMulti(int reqId, const std::string& account, const std::string& modelCode)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support positions multi request.", "");
        return;
    }
...
void EClient::reqAccountUpdatesMulti(
    int reqId, const std::string& account, const std::string& modelCode, bool ledgerAndNLV)
{
    // not connected?
    if (!isConnected())
    {
...
void EClient::reqSecDefOptParams(int reqId, const std::string& underlyingSymbol, const std::string& futFopExchange,
...
void EClient::reqFamilyCodes()
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support family codes requests.", "");
        return;
    }
...
void EClient::reqMatchingSymbols(int reqId, const std::string& pattern)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support matching symbols requests.", "");
        return;
    }
...
void EClient::reqSmartComponents(int reqId, std::string bboExchange)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support smart components request.", "");
        return;
    }
...
void EClient::reqNewsProviders()
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support news providers requests.", "");
        return;
    }
    ...
    const TagValueListSPtr& newsArticleOptions)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support news article requests.", "");
        return;
    }
...
void EClient::reqHistoricalNews(int requestId, int conId, const std::string& providerCodes,
    const std::string& startDateTime, const std::string& endDateTime, int totalResults,
    const TagValueListSPtr& historicalNewsOptions)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support historical news requests.", "");
        return;
    }
...
void EClient::reqHeadTimestamp(
    int tickerId, const Contract& contract, const std::string& whatToShow, int useRTH, int formatDate)
{
    if (!isConnected())
    {
        m_pEWrapper->error(NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg(), "");
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support head timestamp requests.", "");
        return;
    }
...
void EClient::reqHistogramData(int reqId, const Contract& contract, bool useRTH, const std::string& timePeriod)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support histogram requests.", "");
        return;
    }
...
void EClient::reqMarketRule(int marketRuleId)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support market rule requests.", "");
        return;
    }
...
void EClient::reqPnL(int reqId, const std::string& account, const std::string& modelCode)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support PnL requests.", "");
        return;
    }
...
void EClient::reqHistoricalTicks(int reqId, const Contract& contract, const std::string& startDateTime,
    const std::string& endDateTime, int numberOfTicks, const std::string& whatToShow, int useRth, bool ignoreSize,
    const TagValueListSPtr& miscOptions)
{
    if (!isConnected())
    {
...
void EClient::reqTickByTickData(
    int reqId, const Contract& contract, const std::string& tickType, int numberOfTicks, bool ignoreSize)
{
    if (!isConnected())
    {
        m_pEWrapper->error(NO_VALID_ID, NOT_CONNECTED.code(), NOT_CONNECTED.msg(), "");
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support tick-by-tick data request.", "");
        return;
    }
...
void EClient::cancelTickByTickData(int reqId)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support tick-by-tick data cancel.", "");
        return;
    }
...
void EClient::reqCompletedOrders(bool apiOnly)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support completed orders request.", "");
        return;
    }
...
void EClient::reqWshMetaData(int reqId)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support WSHE Calendar API.", "");
        return;
    }
...
void EClient::reqWshEventData(int reqId, const WshEventData& wshEventData)
{
    ...
    {
        if (!wshEventData.filter.empty() || wshEventData.fillWatchlist || wshEventData.fillPortfolio ||
            wshEventData.fillCompetitors)
        {
            m_pEWrapper->error(
                NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support WSH event data filters.", "");
            return;
        }
    }
...
void EClient::reqUserInfo(int reqId)
{
    ...
    {
        m_pEWrapper->error(
            NO_VALID_ID, UPDATE_TWS.code(), UPDATE_TWS.msg() + "  It does not support user info requests.", "");
        return;
    }
