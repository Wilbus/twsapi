...
struct Order
{
    ...
    std::string
        rule80A; // Individual = 'I', Agency = 'A', AgentOtherMember = 'W', IndividualPTIA = 'J', AgencyPTIA = 'U',
                 // AgentOtherMemberPTIA = 'M', IndividualPT = 'K', AgencyPT = 'Y', AgentOtherMemberPT = 'N'
    ...
    int shortSaleSlot =
        0; // 1 if you hold the shares, 2 if they will be delivered from elsewhere.  Only for Action="SSHORT
...
inline void Order::CloneOrderComboLegs(OrderComboLegListSPtr& dst, const OrderComboLegListSPtr& src)
{
    if (!src.get())
        return;
